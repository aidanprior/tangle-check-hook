...
sync_gitattributes() {
  ...
  # Add header comment if file is new or doesn't have our marker
  if ! [ -f "$gitattributes_file" ] || ! grep -q "# Generated by tangle-check-hook" "$gitattributes_file" 2>/dev/null; then
    echo "" >> "$temp_file"
