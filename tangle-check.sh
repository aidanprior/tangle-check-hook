...
tangle_and_stage() {
  ...
  -l org \
    ...
    --eval "(org-babel-tangle-file \"${root}/${ORG_FILE}\")" >/dev/null 2>&1
  git add -A
}
...
sync_gitattributes() {
  ...
  if [ -f "$gitattributes_file" ]; then
    while IFS= read -r line; do
      ...
      has_keep_comment=false
      has_generated_by_comment=false
      [[ "$line" =~ linguist-generated=true ]] && has_generated_marker=true
      [[ "$line" =~ "# tangle-check: keep" ]] && has_keep_comment=true
      [[ "$line" =~ "# Generated by tangle-check-hook" ]] && has_generated_by_comment=true
      if $has_generated_marker && ! $has_keep_comment && ! $has_generated_by_comment; then
        continue
      fi
      echo "$line" >> "$temp_file"
